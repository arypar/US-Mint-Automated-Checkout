...
def send_webhook(webhook_type,site,profile,task_id, cotime):
    if settings.webhook !="":
        webhook = DiscordWebhook(url=settings.webhook, username="Quicksilver", avatar_url="https://i.imgur.com/fy26LbM.png")
        if webhook_type == "OP":
            embed = DiscordEmbed(title="Order Placed",color=0x34c693)
        if webhook_type == "PF":
            embed = DiscordEmbed(title="Payment Declined",color=0x34c693)
        embed.set_author(name="Quicksilver", icon_url = "https://i.imgur.com/fy26LbM.png")
        embed.set_footer(text="Quicksilver - Developed by Mist",icon_url="https://i.imgur.com/fy26LbM.png")
        ...
        embed.add_embed_field(name="Checkout Time", value=cotime[0:3] + " Seconds",inline=True)
        embed.add_embed_field(name="Mode", value="Request",inline=True)
