...
class mintfast:
    def __init__(self,task_id,status_signal,image_signal,product,profile,proxy,monitor_delay,start_time):
        ...
        def addtocartpload():
            ...
            global current_time
            global delay
            self.status_signal.emit({"msg":"Adding to Cart (PRELOAD)","status":"alt"})
            atcchecker = self.session.post(atc, atcdata, headers = HEADERS)
            if atcchecker.text.find("Your Bag is Empty") > -1:
                self.status_signal.emit({"msg":"Error Preloading","status":"error"})
                time.sleep(self.monitor_delay)
                addtocartpload()
        addtocartpload()
        checkpl = self.session.get(cart)
    
        soup = BeautifulSoup(checkpl.text, 'html.parser')
        ...
        removecart = {
            ...
            "format": "ajax",
            "refresh": "cartitemsandsummary",
            "dwfrm_cart_shipments_i0_items_i0_deleteProduct": "Remove"}
        def removepreload():
            preloadremove = self.session.post(checkoutlinkfinal, removecart)
            if str(preloadremove) != "<Response [200]>":
                self.status_signal.emit({"msg":"Error During Preload","status":"error"})
                time.sleep(self.monitor_delay)
                removepreload()
        removepreload()
