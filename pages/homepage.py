...
class HomePage(QtWidgets.QWidget):
    ...
    def setupUi(self, homepage):
        ...
        self.profile_table_header.setGeometry(QtCore.QRect(380, 7, 61, 31))
        ...
        self.status_table_header.setGeometry(QtCore.QRect(570, 7, 61, 31))
        ...
        self.site_table_header.setGeometry(QtCore.QRect(180, 7, 61, 31))
        ...
        self.tasks_header.setGeometry(QtCore.QRect(30, 8, 61, 31))
    ...
    def load_tasks(self):
        ...
        try:
            for task in tasks_data:
                tab = TaskTab(task["site"],task["product"],task["profile"],task["proxies"],task["monitor_delay"],task["start_time"],self.stop_all_tasks,self.scrollAreaWidgetContents)
...
class TaskTab(QtWidgets.QWidget):
    def __init__(self,site,product,profile,proxies,monitor_delay,start_time,stop_all,parent=None):
        super(TaskTab, self).__init__(parent)
        self.task_id = str(int(tasks_total_count.text())+1)
        tasks_total_count.setText(self.task_id)
        self.site,self.product,self.profile,self.proxies,self.monitor_delay,self.start_time,self.stop_all = site,product,profile,proxies,monitor_delay,start_time,stop_all
        self.setupUi(self)
        tasks.append(self)
        tasks_data = return_data("./data/tasks.json")
        task_data = {"task_id": self.task_id,"site":self.site,"product": self.product,"profile": self.profile,"proxies": self.proxies,"monitor_delay": self.monitor_delay,"start_time": self.start_time}
    ...
    def setupUi(self,TaskTab):
        ...
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11) if platform.system() == "Darwin" else font.setPointSize(11*.75)
        font.setBold(False)
        font.setWeight(50)
        ...
        self.profile_label.setGeometry(QtCore.QRect(355, 10, 70, 31))
        ...
        self.status_label.setGeometry(QtCore.QRect(552, 10, 250, 31))
        ...
        self.start_time_label = QtWidgets.QLabel(self.TaskTab)
    ...
    def load_labels(self):
        ...
        self.start_time_label.setText(self.start_time)
    ...
    def update_task(self):
        ...
        self.start_time = self.edit_dialog.start_time.text()
        self.load_labels()
        self.delete_json()
        tasks_data = return_data("./data/tasks.json")
        task_data = {"task_id": self.task_id, "site": self.site, "product": self.product, "profile": self.profile,
                     "proxies": self.proxies, "monitor_delay": self.monitor_delay, "start_time": self.start_time,
                     }
        tasks_data.append(task_data)
        write_data("./data/tasks.json",tasks_data)
...
class TaskThread(QtCore.QThread):
    ...
    def set_data(self,task_id,site,product,profile,proxies,monitor_delay,start_time,max_price):
        self.task_id,self.site,self.product,self.profile,self.proxies,self.monitor_delay,self.start_time,self.max_price = task_id,site,product,profile,proxies,monitor_delay,start_time,max_price
    ...
    def run(self):
        ...
        if proxy == None:
            ...
            return
        if self.site == "US Mint":
            mint(self.task_id,self.status_signal,self.image_signal,self.product,profile,proxy,self.monitor_delay,self.start_time)
        if self.site == "US Mint (FAST)":
            mintfast(self.task_id,self.status_signal,self.image_signal,self.product,profile,proxy,self.monitor_delay,self.start_time)
